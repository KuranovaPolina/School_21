...
int main() {
    ...
    welcome_screen();
    // ф-я завершающего экрана пользуется результатом работы основной функции игры
    final_screen(game());
...
int game() {
    ...
    const int pad_1_x0 = 15;        // горизотальная координата ракетки 1
    const int pad_2_x0 = 65;        // горизотальная координата ракетки 2
    ...
    int pad_2 = pads_y;
    int b_x_dir, b_y_dir = 1;       // направление полёта (1y - вниз, -1y - вверх, 1x - вправо, -1x - влево)
    ...
    while (1) {
        ...
        // от верхнего ракетки 1 летит вверх
        if (ball_x == pad_1_x0 + 1 && (ball_y - pad_1 == -1)) { b_y_dir = -1; }
        // от нижнего ракетки 1 летит вниз
        if (ball_x == pad_1_x0 + 1 && (ball_y - pad_1 == 3)) { b_y_dir = 1; }
        // от верхнего ракетки 2 летит вверх
        if (ball_x == pad_2_x0 - 1 && (ball_y - pad_2 == -1)) { b_y_dir = -1; }
        // от нижнего ракетки 1 летит вниз
        if (ball_x == pad_2_x0 - 1 && (ball_y - pad_2 == 3)) { b_y_dir = 1; }
        ...
        for (int y = 0; y < HEIGTH; ++y) {          // рисуем поляну, цикл строк
            for (int x = 0; x < LENGTH; ++x) {      // вложенный цикл столбцов
                ...
                if (y == 0 || y == HEIGTH - 1) {
                    printf("%c", 61);                                                   // рисуем стенку
                } else if (x == pad_1_x0 && y - pad_1 >= 0 && y - pad_1 < pads_width) {
                    printf("%c", 72);                                                   // рисуем левую ракетку
                } else if (x == pad_2_x0 && y - pad_2 >= 0 && y - pad_2 < pads_width) {
                    printf("%c", 72);                                                   // рисуем правую ракетку
                } else if (x == ball_x && y == ball_y) {
                    printf("%c", 64);                                                   // рисуем мячик
                } else if (x == net) {
                    printf("%c", 124);                                                  // рисуем сетку
...
void welcome_screen() {
    // while (1) {
        ...
        printf("                                                                                \n");
        printf("                                                                                \n");
        printf("                           PONG VERSION V.99 BY ???? TEAM                        \n");
        printf("                                                                                \n");
        printf("                                                                                \n");
        printf("                                                                                \n");
